...
class GALA:
    def __init__(self, input_list=None, file=None, verbose=False, p=3, b=0.5, ccr=0.5):
        ...
        self.GALAgrouping (self.graph, self.iterations, self.root_node, self.final_node)
    ...
    def GALAgrouping (self, graph, iterations, root_node, final_node):
        ...
        ## Phase 1: Constructing network of LA Isomorphic to dependent taskes grouped DAG
        for nodei in graph:
            for nodej in graph:
                if graph[nodei.id][nodej.id] != -1:   
                    pre = set(graph.predecessors(nodej)) | set(graph.predecessors(nodej))
                    suc = set(graph.successors(nodei)) | set(graph.successors(nodej))
                    name = str(nodei) + '/' + str(nodej)
                    graph. dd_edges_from([(p, name) for p in pre])
                    graph.add_edges_from([(name, s) for s in suc])
                    graph.remove_nodes_from([nodei, nodej])            
        Automatas = dict()
        action_set = dict()
        action_prob = dict()
        reward = dict()
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    new_sch = GALA(file=args.input, verbose=True, p=4, b=0.1, ccr=0.1)
    print(new_sch)